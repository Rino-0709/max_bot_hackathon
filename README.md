...
SCANNER_TEST_ACCESS_TOKEN=change_me_demo_scanner_token
SCANNER_PUBLIC_URL=https://scanner.aksenovaks.online/scanner
ADMIN_USER_IDS=
...
Grafana автоматически подключает Prometheus и загружает dashboard `Электронное бюро пропусков overview`.
...
Fallback-порты `8081` и `8443` по умолчанию привязаны к `127.0.0.1`, чтобы наружу был открыт только домен на HTTPS.

Для обычной работы администратор открывает в боте `Меню админа` -> `Ключ сканера`. Бот отправляет личную ссылку и ключ. Все проходы, подтверждённые через этот ключ, записываются на конкретного администратора.

`SCANNER_ACCESS_TOKEN` остаётся запасным сервисным ключом из `.env`. Для демонстрации есть команда `/test`: она отправляет общий тестовый ключ, ссылку на сканер и тестовый QR-код. Этот режим нужен только для показа проекта и не заменяет личные ключи администраторов.

На сервере порт по умолчанию закрыт на `127.0.0.1`, поэтому для проверки с локального компьютера используйте SSH-туннель:

```powershell
ssh -L 8080:127.0.0.1:8080 root@SERVER_IP
```

После проверки QR сканер показывает данные заявки, статус, дату, корпус и количество проходов. Кнопка `Подтвердить проход` фиксирует первый или повторный проход так же, как администратор в боте. В истории проходов сохраняется, кто именно подтвердил проход.

Камера телефона обычно требует HTTPS. Для домена `scanner.aksenovaks.online` используется обычный TLS-сертификат Let's Encrypt. Адреса по IP оставлены как запасные.
...
- `/test` - временная команда для демонстрации QR-сканера.
...
В списке slash-команд бота не должно быть административных команд. Оставьте только пользовательские команды: `/start`, `/whoami`, `/reset_session`, `/test`. После демонстрации `/test` можно убрать из списка команд и кода.
