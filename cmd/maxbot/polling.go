...
func (app *App) showDataPolicy(ctx context.Context, bctx BotContext, user UserRow) error {
	text := strings.Join([]string{
		...
		"",
		"Срок хранения: на период работы сервиса и последующей технической очистки. Для промышленной эксплуатации срок должен быть закреплён внутренним регламентом.",
		"",
